...
- **Skip link.** Both layouts start `<body>` with `<a class="skip-link" href="#main-content">`, and both `<main>` elements carry `id="main-content" tabindex="-1"` — the `tabindex="-1"` is required, not decorative: without it, activating the skip link scrolls the page but never actually moves keyboard focus, so a screen reader user lands back at the top on the next Tab press. If a new top-level layout is ever added, it needs both pieces, not just the visible link.
...
- No client-side JavaScript framework or heavy dependency, analytics script, or third-party embed without a real reason. The site does ship a small amount of vanilla JS now (the random-logo picker, the theme toggle, and the blog page's tag-filter/search, each a few lines with zero dependencies) — that bar is "vanilla, minimal, and justified," not "zero JS at all costs." Syntax highlighting in code blocks is the opposite case done right: `@11ty/eleventy-plugin-syntaxhighlight` runs entirely at build time (Node-side, via Prism) and ships zero bytes of JS to the browser — prefer that shape (a build-time devDependency) over a client-side library whenever the output can be fully static.
