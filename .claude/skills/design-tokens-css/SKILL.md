...
Every heading and body size in the file is set in `rem`, on purpose: `rem` is relative to the root (`<html>`) element's font-size, not the browser default directly. The site's base size is set once, on `html { font-size: 118.75%; }` (a percentage of the visitor's own browser default, not a hardcoded px value — respects a visitor's own text-size preference instead of overriding it). If reading comfort ever needs to change again, adjust that one line rather than touching individual heading/body rules — every `rem` value in the file scales proportionally for free. Note that the reading-content rules (`.post p`/`.post li`, `.hero-bio`, `.post-list p`) were *also* bumped a bit beyond what the proportional scale alone gives them (they're not simple multiples of the root change) — user feedback specifically wanted the actual prose bigger, not just everything uniformly bigger, so don't "simplify" those back to a clean ratio.
...
Syntax highlighting (`.token.*` rules in `style.css`, driven by `@11ty/eleventy-plugin-syntaxhighlight` at build time) deliberately reuses just these four tokens — `--text-primary`/`--text-muted`/`--accent`/`--accent-hover` — differentiated by weight and italics, instead of the 6-8 arbitrary hues a typical Prism theme uses. Don't add a `--syntax-*` color palette; if code needs more visual distinction later, that's a conversation with the user about whether the "every color traces to an existing token" rule should bend for code blocks specifically, not a unilateral call.
